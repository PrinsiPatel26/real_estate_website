...
function initialRecord(resource: string): RecordValue {
  ...
  if (resource === 'blogs') return { title: '', slug: '', category: '', excerpt: '', author: 'Chauhan Realtors', content: [], image: '', isPublished: true };
...
export function AdminResourcePage() {
  ...
  const filteredRecords = useMemo(() => records.filter((record) => displayName(record).toLowerCase().includes(query.toLowerCase()) || String(record.slug || '').toLowerCase().includes(query.toLowerCase())), [records, query]);
  ...
  return <div className="min-h-screen bg-[#f2efe8] text-[#151515]">
    ...
    <main className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-12">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-[0.66rem] font-semibold uppercase tracking-[0.2em] text-[#a98232]">Admin CMS</p><h1 className="mt-3 font-display text-4xl font-medium">{label}</h1></div><button type="button" onClick={startCreate} className="inline-flex h-11 items-center justify-center gap-2 bg-[#c9a227] px-5 text-[0.68rem] font-semibold uppercase tracking-[0.16em]"><PlusIcon className="h-4 w-4" />Add {label.slice(0, -1)}</button></div>
