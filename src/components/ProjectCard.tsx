...
export function ProjectCard({ project, delay = 0 }: ProjectCardProps) {
  ...
  return (
    ...
    }} className="group box-border flex h-[360px] w-[calc(100vw-2rem)] min-w-[calc(100vw-2rem)] max-w-[380px] shrink-0 snap-start cursor-pointer flex-col break-words overflow-hidden border border-[#d4af37]/25 bg-[#151515] text-white transition-all duration-300 ease-lux hover:-translate-y-1 hover:border-[#c9a24a] hover:shadow-[0_18px_36px_rgba(0,0,0,0.18)] md:h-full md:w-auto md:min-w-0 md:max-w-none md:shrink">
...
export function ProjectGrid({ projects, featured = false }: { projects: Project[]; featured?: boolean }) {
  return <div className={`mx-auto flex w-full items-stretch ${featured ? 'max-w-[78rem]' : 'max-w-[62rem]'} snap-x snap-mandatory gap-4 overflow-x-auto overflow-y-hidden px-4 pb-2 scroll-smooth [scrollbar-width:none] [-webkit-overflow-scrolling:touch] [overscroll-behavior-x:contain] [&::-webkit-scrollbar]:hidden md:grid md:grid-cols-2 md:gap-4 md:overflow-visible md:px-0 md:pb-0 md:snap-none ${featured ? 'lg:grid-cols-4' : 'lg:grid-cols-3'}`}>{projects.map((project, index) => <ProjectCard key={project.slug} project={project} delay={index * 0.04} />)}</div>;
}
