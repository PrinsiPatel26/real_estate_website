...
export function TestimonialsSection({ fullPage = false }: TestimonialsSectionProps) {
  ...
  const toggleTestimonial = (id: string) => {
    setExpandedTestimonialId((currentId) => currentId === id ? null : id);
  };
    ...
    <section id="testimonials" className="overflow-hidden bg-[#f5f3ee] py-20 text-[#111827] sm:py-24 lg:py-28">
      <div className="mx-auto max-w-[1280px] px-5 sm:px-8 lg:px-10">
        ...
        <div className="relative mt-10 sm:mt-12">
          <button type="button" onClick={() => moveCarousel(-1)} disabled={!canScrollLeft} aria-label="Previous testimonial" className="absolute left-0 top-1/2 z-10 hidden h-11 w-11 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-[#d4af37]/60 bg-[#0a0a0a] text-[#d8b968] shadow-[0_3px_12px_rgba(0,0,0,0.12)] transition hover:scale-105 hover:border-[#d4af37] hover:bg-[#d4af37] hover:text-[#111111] disabled:pointer-events-none disabled:opacity-35 md:flex">
            <ArrowLeftIcon className="h-4 w-4" aria-hidden="true" />
          </button>
          <div ref={carouselRef} className="testimonial-carousel-viewport flex items-start gap-6 overflow-x-auto px-0 pb-3 scroll-smooth md:px-6">
            {testimonials.map((item) => <TestimonialCard key={item.id} item={item} expanded={expandedTestimonialId === item.id} onToggle={toggleTestimonial} />)}
          </div>
          <button type="button" onClick={() => moveCarousel(1)} disabled={!canScrollRight} aria-label="Next testimonial" className="absolute right-0 top-1/2 z-10 hidden h-11 w-11 translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-[#d4af37]/60 bg-[#0a0a0a] text-[#d8b968] shadow-[0_3px_12px_rgba(0,0,0,0.12)] transition hover:scale-105 hover:border-[#d4af37] hover:bg-[#d4af37] hover:text-[#111111] disabled:pointer-events-none disabled:opacity-35 md:flex">
            <ArrowRightIcon className="h-4 w-4" aria-hidden="true" />
          </button>
        </div>

        <div className="mt-2 flex justify-center gap-3 md:hidden">
          <button type="button" onClick={() => moveCarousel(-1)} disabled={!canScrollLeft} aria-label="Previous testimonial" className="flex h-10 w-10 items-center justify-center rounded-full border border-[#d4af37]/60 bg-[#0a0a0a] text-[#d8b968] shadow-[0_3px_12px_rgba(0,0,0,0.12)] transition hover:border-[#d4af37] hover:bg-[#d4af37] hover:text-[#111111] disabled:opacity-35"><ArrowLeftIcon className="h-4 w-4" aria-hidden="true" /></button>
          <button type="button" onClick={() => moveCarousel(1)} disabled={!canScrollRight} aria-label="Next testimonial" className="flex h-10 w-10 items-center justify-center rounded-full border border-[#d4af37]/60 bg-[#0a0a0a] text-[#d8b968] shadow-[0_3px_12px_rgba(0,0,0,0.12)] transition hover:border-[#d4af37] hover:bg-[#d4af37] hover:text-[#111111] disabled:opacity-35"><ArrowRightIcon className="h-4 w-4" aria-hidden="true" /></button>
        </div>
      </div>
    </section>
  );
...
function TestimonialCard({ item, expanded, onToggle }: { item: (typeof testimonials)[number]; expanded: boolean; onToggle: (id: string) => void }) {
  const initials = item.name.split(' ').map((part) => part[0]).slice(0, 2).join('');
  return <article data-testimonial-card className="flex min-h-[300px] shrink-0 basis-full snap-start flex-col rounded-[14px] border border-[#e5e7eb] bg-white p-6 shadow-[0_3px_14px_rgba(0,0,0,0.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_8px_22px_rgba(0,0,0,0.09)] md:basis-[calc(50%_-_0.75rem)] lg:basis-[calc(33.333333%_-_1rem)] sm:p-7">
